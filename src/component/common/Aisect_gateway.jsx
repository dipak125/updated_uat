...
class Aisect_gateway extends Component{
    constructor(props) {
        super(props);
    ...
    handleSubmit = (values)=>{
        var policy_ref_no = this.state.policyHolder_refNo ? this.state.policyHolder_refNo : '0'
        ...
        axios.post('aisect/payment',formData)
        .then(resp =>{
            //let decryptResp = JSON.parse(encryption.decrypt(res.data))
            console.log(resp);
            if(resp.data.error == false){	
                this.policyInception(resp.data.data.transaction_no, this.state.quote_id)
            }else{
                swal(resp.data.msg)
    ...
    policyInception = (transaction_no ) => {
        ...
        //this.props.history.push(`/ThankYou/${resp.data.data.PolicyNo}?access_id=${this.state.policyHolder_refNo}`)
    }
...
export default withRouter(
    connect(mapStateToProps, mapDispatchToProps)(Aisect_gateway)
);