...
class RegistrationPCV_TP extends Component {
    state = {
        motorInsurance: '',
        regno: '',
        length: 14,
        ...
        if (policyHolder_id > 0) {
            if (sessionStorage.getItem('csc_id')) {
                post_data = {
                    ...
                    'page_name': `Registration_PCV_TP/${productId}`
                }
            }
            else {
                post_data = {
            ...
            this.props.loadingStart();
            axios
                .post(`pcv-tp/update-registration`, formData)
                .then(res => {
                    let decryptResp = JSON.parse(encryption.decrypt(res.data))
                    console.log("decrypt", decryptResp)
            ...
            this.props.loadingStart();
            axios
                .post(`pcv-tp/registration`, formData)
                .then(res => {
                    let decryptResp = JSON.parse(encryption.decrypt(res.data))
                    console.log("decrypt", decryptResp)
                    this.props.loadingStop();
...
export default withRouter(connect(mapStateToProps, mapDispatchToProps)(RegistrationPCV_TP));