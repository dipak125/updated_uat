...
class TwoWheelerSelectBrand extends Component {
    ...
    fetchData = () => {
        ...
        axios.get(`two-wh/details/${policyHolder_id}`)
            .then(res => {
                ...
                this.setState({
                    motorInsurance, vehicleDetails, fastlanelog,is_fieldDisabled
                })
                this.getBrands();
            })
            .catch(err => {
        ...
        if(policyHolder_id > 0 ) {
            if(sessionStorage.getItem('csc_id')) {
                post_data = {
                    ...
                    //'fastlaneLog_id': this.state.fastlaneLogId ? this.state.fastlaneLogId : fastlanelog && fastlanelog.id ? fastlanelog.id : "",
                    'fastlaneLog_id': this.state.fastLaneData ? this.state.fastLaneData.fastlaneLog_id : "",
    ...
    render() {
        console.log("log",this.state.flag)
        const { brandList, motorInsurance, selectedBrandDetails, brandModelList,fuelType, fastLaneData, selectedBrandId,
            selectedModelId, selectedVarientId, otherBrands, vehicleDetails, error_msg, brandName, modelName, brandView,flag,is_fieldDisabled } = this.state
        const { productId } = this.props.match.params
