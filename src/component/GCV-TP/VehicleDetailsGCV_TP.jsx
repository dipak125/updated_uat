...
class VehicleDetailsGCV extends Component {
       ...
       const {is_fieldDisabled} =this.state
        if (is_fieldDisabled && is_fieldDisabled == "true") {
            ...
            this.props.history.push({
                pathname: `/SelectBrand_GCV_TP/${productId}`,
                appState: {
                  flag : 1
  ...
  //--------------------------------------------------------
    ...
    handleSubmit = (values, actions) => {
        ...
        let post_data = {}
        if(is_fieldDisabled == "true" || values.policy_type_id == '2') {
            post_data = {
    ...
    fetchData = () => {
        ...
        axios.get(`gcv-tp/policy-holder/details/${policyHolder_id}`)
            .then(res => {
                ...
                this.setState({
                    motorInsurance, previousPolicy, vehicleDetails,RTO_location, previous_is_claim,is_fieldDisabled
                })
                this.props.loadingStop();
            })
            .catch(err => {
                                ...
                                return (
                                    <Form>
                                        <Row>
                                            ...
                                            {(values.policy_type_id == '2' && values.valid_previous_policy == '1') || is_fieldDisabled =="true" ?
                                                <Fragment>
                                                <Row>
                                                    <Col sm={12}>
                                                        <FormGroup>
