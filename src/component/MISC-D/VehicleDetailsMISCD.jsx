    ...
    selectBrand = (productId) => {
        ...
        if (is_fieldDisabled && is_fieldDisabled == "true") {
            ...
            this.props.history.push({
                pathname: `/SelectBrand_MISCD/${productId}`,
                appState: {
                  flag : 1
  ...
  //--------------------------------------------------------
    ...
    handleSubmit = (values, actions) => {
        ...
        .then(res => { 
            ...
            if(decryptResp.error == false){
                this.props.history.push(`/OtherComprehensive_MISCD/${productId}`);
            }
            else{
    ...
    fetchData = () => {
        ...
        axios.get(`miscd/policy-holder/details/${policyHolder_id}`)
            .then(res => {
                ...
                this.setState({
                    motorInsurance,is_fieldDisabled ,previousPolicy, vehicleDetails,RTO_location, previous_is_claim, no_of_claim
                })
                this.props.loadingStop();
            })
            .catch(err => {
    ...
    render() {
        ...
        const {insurerList, showClaim, previous_is_claim, motorInsurance, previousPolicy,CustomerID,suggestions,
              vehicleDetails, RTO_location, averagemonthlyusages,goodscarriedtypes,permittypes, location_reset_flag,is_fieldDisabled} = this.state
